...
func doInNextRunLoop(_ job: @escaping () -> Void) {
    DispatchQueue.main.asyncAfter(deadline: DispatchTime.now(), execute: job)
}

final class YepAudioService: NSObject {
    
    static let shared = YepAudioService()
    ...
    func playAudioWithMessage(_ message: Message, beginFromTime time: TimeInterval, delegate: AVAudioPlayerDelegate, success: () -> Void) {
        ...
        if AVAudioSession.sharedInstance().category == AVAudioSessionCategoryRecord {
            do {
                ...
                print("playAudioWithMessage setCategory failed: \(error)")
            }
        }

        if let audioFileURL = message.videoFileURL {
            do {
...
//                    }
                ...
                print("play audio error: \(error)")
            }

        } else {
            print("please wait for download") // TODO: Download audio message, check first
        }
    }
...
//        } else {
//            print("please wait for download") // TODO: Download feed audio, check first
//        }
    }
    ...
    func playOnlineAudioWithFeedAudio(_ feedAudio: FeedAudio, beginFromTime time: TimeInterval, delegate: AVAudioPlayerDelegate, success: () -> Void) {
        ...
        print("playOnlineAudioWithFeedAudio")
