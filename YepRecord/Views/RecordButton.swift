...
final class RecordButton: UIButton {
    ...
    private lazy var _bounds: CGRect = {
        let origin = CGPoint.zero
        return CGRect(origin: origin, size: self.intrinsicContentSize)
    }()

    lazy var outerPath: UIBezierPath = {
        return UIBezierPath(ovalIn: self._bounds.insetBy(dx: 8, dy: 8))
    }()

    lazy var innerDefaultPath: UIBezierPath = {
        return UIBezierPath(roundedRect: self._bounds.insetBy(dx: 14, dy: 14), cornerRadius: 43)
    }()

    lazy var innerRecordingPath: UIBezierPath = {
        return UIBezierPath(roundedRect: self._bounds.insetBy(dx: 35, dy: 35), cornerRadius: 5)
    }()
